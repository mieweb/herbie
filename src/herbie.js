...
Either set a variable or chain your methods.
...
$('#myele').whatever(...).whatever(...).whatever(...);


3. MIE has standards regarding coding style.  You should learn them. http://sonar.med-web.com:9000/coding_rules#languages=js|s=createdAt|asc=false

Most notably:
	a. always use curly braces for "if/else/for/while/do" statements: http://sonar.med-web.com:9000/coding_rules#rule_key=javascript%3ACurlyBraces
	b. single quotes should be used for string literals http://sonar.med-web.com:9000/coding_rules#rule_key=javascript%3ASingleQuote
	c. sections of code should not be "commented out" http://sonar.med-web.com:9000/coding_rules#rule_key=javascript%3ACommentedCode
	d. "===" and "!==" should be used instead of "==" and "!="

	
4. jQuery selector performance: http://24ways.org/2011/your-jquery-now-with-less-suck

jQuery extends native DOM locators/selectors (getElementsByTagName, getElementsByClassName etc) with additional tools (like :contains()) but applying a non-native selector onto a native selector makes the entire selector into a non-native selector and MUCH SLOWER.  Selectors should be separated so the non-native portion only has to search the subset already returned from the native portion.

before:
$('label:contains(sometext)');

after:
$('label').filter(':contains(sometext)');
  ^native         ^non-native


5. This file has mixed indention (tabs and spaces on the same line!). I'll be fixing that as well. Please fix whatever editor you're using to be consistent.
